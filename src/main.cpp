...
namespace
{
    ...
    void parseOFFandPolyhedron(string source) {
        printf ("%s \n", "A string");
        /*
        // create and read Polyhedron
        Polyhedron mesh;
        ifstream input("data/cactus.off");
        if ( !input || !(input >> mesh) || mesh.empty() ) {
            cerr << "Not a valid off file." << std::endl;
            //return EXIT_FAILURE;
        }
        ...
        cout << "Number of segments: " << number_of_segments << std::endl;

        for(Facet_iterator facet_it = mesh.facets_begin(); facet_it != mesh.facets_end(); ++facet_it) {
            cout << segment_property_map[facet_it] << " ";
        }
        */
    }
